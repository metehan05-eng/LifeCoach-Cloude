...
/* ── YouTube Video Processing Indicator ── */
const YouTubeProcessingIndicator = ({ videoId }) => {
  const [phase, setPhase] = React.useState(0);
  React.useEffect(() => {
    const interval = setInterval(() => setPhase(p => (p + 1) % 3), 800);
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: '13px', fontWeight: 700, color: '#fcd34d' }}>
          Video Çözümleniyor{dots}
        </div>
        <div style={{ fontSize: '11px', color: 'rgba(245,158,11,0.6)', marginTop: '2px' }}>
          Ses alınıyor ve yapay zeka ile analiz ediliyor...
        </div>
      </div>
      <div style={{
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '18px' }}>
       ...
       {/* YouTube Video Link (no iframe — prevents hydration errors) */}
       {data.youtube_id && (
         <a
         ...
         >
           ...
           <div style={{ flex: 1 }}>
             <div style={{ fontSize: '13px', fontWeight: 700 }}>İlgili Video</div>
             <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.45)', marginTop: '2px' }}>
               YouTube'da izlemek için tıkla ↗
             </div>
           </div>
         </a>
       )}
        ...
        {videos.slice(0, 3).map((video, idx) => (
          ...
          >
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              style={{ textDecoration: 'none', display: 'flex', gap: '12px', padding: '12px', alignItems: 'stretch' }}
            >
              <div
                style={{
                  position: 'relative', width: '132px', minWidth: '132px', height: '74px',
                  borderRadius: '12px', overflow: 'hidden', background: '#111', flexShrink: 0,
                }}
              >
                {video.thumbnail ? (
              ...
              <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <div style={{ color: '#fecaca', fontSize: '14px', fontWeight: 700, lineHeight: 1.4 }}>
                  {video.title}
                </div>
  ...
  return (
    <div style={{ marginTop: '12px' }}>
      <div style={{
        ...
        <span style={{ fontSize: '13px' }}>🖼️</span> {topicLabels[image.topic] || 'Görsel'}
      </div>
      <div style={{
        ...
        <a href={image.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
          <div style={{
            ...
            background: 'rgba(0,0,0,0.15)', overflow: 'hidden', cursor: 'pointer',
          }}>
            <img src={image.src.large} alt={image.alt} loading="lazy"
              onLoad={() => setLoaded(true)}
              style={{
                position: 'absolute', top: 0, left: 0, width: '100%', height: '100%',
                objectFit: 'cover', opacity: loaded ? 1 : 0, transition: 'opacity 0.4s ease',
              }} />
            {!loaded && <div style={{
        ...
        <div style={{
          ...
          justifyContent: 'space-between', borderTop: '1px solid rgba(236,72,153,0.1)',
        }}>
          ...
          <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.2)' }}>Pexels</span>
        </div>
      </div>
      {images.length > 1 && (
          ...
          {images.slice(1).map(img => (
            <a key={img.id} href={img.url} target="_blank" rel="noopener noreferrer"
              style={{ flexShrink: 0, width: '80px', height: '60px', borderRadius: '8px',
                overflow: 'hidden', border: '1px solid rgba(236,72,153,0.15)', textDecoration: 'none' }}>
      ...
      <div style={{
        ...
        <div className="chat-user-bubble" style={{
          ...
          letterSpacing: '-0.01em',
           ...
           {/* YouTube video processing indicator from message content */}
           {(() => {
             if (message.content) {
               const ytId = extractYouTubeVideoId(message.content);
               if (ytId) return <YouTubeProcessingIndicator videoId={ytId} />;
             }
             return null;
           })()}
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {message.pexelsImages && message.pexelsImages.length > 0 && (
          <PexelsCard images={message.pexelsImages} />
        )}
