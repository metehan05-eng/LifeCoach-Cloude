...
export async function optimizeWafflePrompt(prompt, options = {}) {
  ...
  // Önce Qwen dene (ana motor)
  try {
    const { callQwen } = await import('./qwen-api.js');
    const sysMsg = `You are an elite AI art director and prompt engineer. Translate user idea to ONE highly detailed English generation prompt. Output ONLY the prompt.`;
    const usrMsg = `Mode: ${mode}\nUser request: ${basePrompt}\n\nPerfect generation prompt:`;
    const optimized = await callQwen([{ role: 'system', content: sysMsg }, { role: 'user', content: usrMsg }], { model: 'qwen-turbo', max_tokens: 600 });
    if (optimized) return { optimizedPrompt: optimized.slice(0, 1200), engine: 'qwen' };
  } catch (qwenErr) {
    console.warn('[Waffle Qwen] Prompt optimize failed, trying HF:', qwenErr.message);
  }
...
export async function generateWaffleImage(optimizedPrompt, options = {}) {
  ...
  // Önce Qwen Wan motoru dene
  try {
    const { generateWanImage, generateWanImageVariation } = await import('./qwen-vision.js');
    if (referenceImage) {
      const ref = parseBase64Image(referenceImage);
      if (ref) {
        const imgUrl = `data:${ref.mime};base64,${ref.base64}`;
        return await generateWanImageVariation(imgUrl, prompt, { size: `${width}*${height}` });
      }
    }
    return await generateWanImage(prompt, { size: `${width}*${height}` });
  } catch (wanErr) {
    console.warn('[Waffle Wan] Görsel üretimi başarısız, HF deneniyor:', wanErr.message);
  }
...
export async function generateWaffleVideo(optimizedPrompt, options = {}) {
  ...
  const prompt = (optimizedPrompt || '').trim();
  if (!prompt) throw new Error('Video için prompt gerekli.');

  // Önce Qwen Wan2.1 motoru dene
  try {
    const { generateWanVideo, generateWanVideoFromImage } = await import('./qwen-vision.js');
    if (referenceImage) {
      const ref = parseBase64Image(referenceImage);
      if (ref) {
        const imgUrl = `data:${ref.mime};base64,${ref.base64}`;
        return await generateWanVideoFromImage(imgUrl, prompt, { ratio: width > height ? '16:9' : '9:16', resolution: '720P' });
      }
    }
    return await generateWanVideo(prompt, { ratio: width > height ? '16:9' : '9:16', resolution: '720P' });
  } catch (wanErr) {
    console.warn('[Waffle Wan] Video üretimi başarısız, HF deneniyor:', wanErr.message);
  }

  if (!hf) throw new Error('Video üretimi için HF_TOKEN veya DASHSCOPE_API_KEY gerekli.');
