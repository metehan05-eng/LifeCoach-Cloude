 ...
 */
export async function callQwenModel(messages, maxTokens = 3000, options = {}) {
...
{
  "shortTitle": "Hedefin 3-5 kelimelik kısa, akılda kalıcı başlığı (ör: Mobil Uygulama MVP'si)",
  "summary": "Hedefin derinlemesine analizi, kullanıcıyı motive eden ve stratejiyi açıklayan en az 3-4 cümlelik düz metin özet.",
  "aiSummary": "AI koçun hedefe dair kısa değerlendirmesi: zorluk seviyesi, potansiyel engeller, motivasyon notu (2-3 cümle).",
  "steps": [
    {
      "order": 1,
...
// ─────────────────────────────────────────────────────────────
// 🚀 Startup Yol Haritası – Kategori Bazlı Analiz + MVP Fazları + Yalın Kanvas
// ─────────────────────────────────────────────────────────────
...
Kullanıcının iş fikrini analiz et ve aşağıdaki 4 kategoriden birine atayarak o kategoriye özel bir yol haritası üret.

İLK ADIM: Kullanıcının girdisini oku ve en uygun kategoriyi seç:
- SAAS_SOFTWARE: Web/Mobil uygulama, Yapay Zeka aracı, B2B/B2C yazılım, dijital platform.
- E_COMMERCE: E-ticaret mağazası, Dropshipping, Pazaryeri satıcılığı (Trendyol/Amazon), kendi markasını satma.
- PHYSICAL_BUSINESS: Kafe, Restoran, Spor salonu, Danışmanlık ofisi, kuaför, fiziksel mağaza.
- CONTENT_CREATION_AGENCY: YouTube/TikTok içerik üretimi, Dijital Ajans, Freelance hizmet, influencer.

KATEGORİYE GÖRE AŞAĞIDAKİ KURALLARI UYGULA:

EĞER kategori SAAS_SOFTWARE ise:
Fazlar: MVP Geliştirme → Kullanıcı Deneyimi & UI/UX → Topluluk & Test → Büyüme & Gelir Modeli.
techStack doldur (framework, db, hosting vb.).
Maliyetler: Sunucu, API harcamaları, Geliştirici giderleri.

EĞER kategori E_COMMERCE ise:
Fazlar: Tedarik & Ürün Seçimi → Şirket & Sanal POS Kurulumu → Mağaza Altyapısı (Shopify/ikas) → Dijital Pazarlama & Sosyal Medya → Meta Ads / Influencer Reklamları & Satış.
techStack alanını BOŞ BIRAK ([]).
Maliyetler: Tedarik maliyeti, Reklam bütçesi, Kargo & Komisyon, Altyapı aboneliği.

EĞER kategori PHYSICAL_BUSINESS ise:
Fazlar: Lokasyon Analizi & Kiralama → Ruhsat & Şirket Açılışı → Tedarikçi & Ekipman Alımı → Mekan Tasarımı → Yerel Pazarlama & Açılış.
techStack alanını BOŞ BIRAK ([]).
Maliyetler: Kira, Depozito, Tadilat, Personel maaşları, Yerel reklamlar.

EĞER kategori CONTENT_CREATION_AGENCY ise:
Fazlar: Niş (Niche) Belirleme → Ekipman & Stüdyo Kurulumu → İçerik Stratejisi → Yayın & Etkileşim → Sponsorluk & Monetizasyon.
techStack alanını BOŞ BIRAK ([]).
Maliyetler: Ekipman maliyeti, Montaj/Kurgu araçları, Pazarlama.

ÇIKTI FORMATI (SADECE bu JSON, başka metin yok):
{
  "category": "SAAS_SOFTWARE | E_COMMERCE | PHYSICAL_BUSINESS | CONTENT_CREATION_AGENCY",
  "analysis": {
    ...
    "techStack": [] // SADECE SAAS_SOFTWARE için doldur, diğerlerinde boş array
  },
  "mvpPhases": [
    {
      "phase": "Faz 1: ...",
      "title": "Aşamanın net başlığı",
      "tasks": ["Görev 1", "Görev 2", "Görev 3"]
    }
  ],
  "leanCanvas": {
    "problems": ["Sorun 1", "Sorun 2", "Sorun 3"],
    "solutions": ["Çözüm 1", "Çözüm 2"],
    "revenues": ["Gelir modeli 1", "Gelir modeli 2"],
    "costs": ["Kategoriye uygun maliyet kalemi"] // sektöre göre değişmeli!
  }
}

KURALLAR:
- MVP fazları 3-5 arası olmalı. Her fazda 3-6 görev.
- Lean Canvas maliyet kalemleri sektöre TAM uygun olmalı (yazılım değilse "sunucu" yazma).
- Faz adları ve görevler kategoriye özgü, gerçekçi ve uygulanabilir olmalı.
- Türkiye pazarı odaklı düşün.`;

  const messages = [
    { role: "system", content: systemPrompt },
    ...chatHistory,
    { role: "user", content: `Startup fikrim: "${ideaDescription}"\n\nÖnce kategoriyi belirle, ardından o kategoriye uygun yol haritasını oluştur.` },
  ];
