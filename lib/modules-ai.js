...
Kullanıcının girdiği soyut, büyük veya karmaşık hedefi al; onu derinlemesine analiz ederek n8n otomasyon tarzı dikey bir akış şemasına, net bir özete ve haftalık bir programa dönüştür.

KRITIK KURAL:
- Yanitinda KESINLIKLE markdown kod bloklari, giris metni, kapanis metni veya herhangi bir duz yazi BULUNMAMALIDIR.
- Ciktin SADECE asagidaki JSON semasina birebir uyan, dogrudan JSON.parse() ile cozulebilecek ham bir JSON string olmalidir.

Kullanıcı hedefini okuduktan sonra üreteceğin JSON:
{
  "summary": "Hedefin derinlemesine analizi, kullanıcıyı motive eden ve stratejiyi açıklayan en az 3-4 cümlelik düz metin özet.",
  "steps": [
    {
      "order": 1,
      "title": "Aşamanın Kısa ve Net Başlığı (n8n kutucuğunun üstünde görünecek)",
...
Hedefleri bölerken mantıklı, gerçekçi ve uygulanabilir adımlar seç. Steps 4-7 adım olmalı. WeeklyPlans 2-4 hafta olmalı.`;
  ...
  const messages = [
    ...
    ...chatHistory,
    { role: "user", content: `Hedefim: "${targetText}"\n\nBu hedef için yukarıdaki formata birebir uyan JSON üret.` },
  ];
...
Kullanıcının ikilemini analiz et, Seçenek A ve B'yi belirle.
SADECE aşağıdaki JSON şemasını kullan, ekstra alan ekleme:

{
  "optionA": "Seçenek A'nın net adı — örn: 'Kurumsal işe devam etmek'",
  "optionB": "Seçenek B'nin net adı — örn: 'İstifa edip SaaS projesine odaklanmak'",
  "prosCons": {
    "optionA": {
  ...
  "simulation": {
    "optionA": { "m3": "Mevcut düzen devam eder.", "m6": "Rutin can sıkmaya başlar.", "m12": "Keşkeler çoğalabilir." },
    "optionB": { "m3": "İlk MVP yayında olur, stres yüksek.", "m6": "Kullanıcı kazanımı başlar.", "m12": "Finansal özgürlük potansiyeli." }
  },
  "riskScores": { "optionA": 30, "optionB": 70 },
  "coachVerdict": "LifeCoach AI'ın iki seçeneği de rasyonel olarak değerlendiren, nihai stratejik koçluk tavsiyesi ve kapanış özeti."
}

Kurallar:
- pros/cons score değerleri 1-10 arası olmalı (10 = en önemli)
- Her seçenek için en az 2, en fazla 5 madde olmalı
- simulation metinleri kısa (1 cümle) ve net olmalı
- riskScores 0-100 arası (0 = risksiz, 100 = çok riskli)
- coachVerdict 2-4 cümle, motive edici ve stratejik olmalı`;
