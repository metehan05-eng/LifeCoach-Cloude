...
export function mapFunctionToToolCall(name, args) {
  switch (name) {
    ...
    default:
      // Plugin tool fallback — route unknown functions directly
...
export async function runDeepSeekWithTools({
  ...
  signal,
  pluginTools,
  onlyPluginTools = false,
}) {
  const tools = pluginTools?.length
    ? onlyPluginTools
      ? pluginTools
      : [...DEEPSEEK_FUNCTION_SCHEMAS, ...pluginTools]
    : DEEPSEEK_FUNCTION_SCHEMAS;
