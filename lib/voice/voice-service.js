...
export class VoiceService {
  ...
  async _speakExternalTTS(text, { onStart, onEnd, onError, lang } = {}) {
    try {
      const res = await fetch('/api/tts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: text.slice(0, 500), language: lang || this.lang }),
      });
      ...
      console.warn('[ExternalTTS] Fallback to browser TTS:', err.message);
      onError?.(err);
  ...
  async speak(text, { onStart, onEnd, onError, lang } = {}) {
    ...
    if (this.provider === "elevenlabs" || this.provider === "external") {
      return this._speakExternalTTS(text, { onStart, onEnd, onError, lang });
    }
