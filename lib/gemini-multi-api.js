...
// Varsayılan Modeller (Custom Vertex AI Modeli Birincil Yapıldı)
const DEFAULT_MODELS = [
    'custom-vertex-ai', // Google Cloud Custom Endpoint
...
export async function callOpenRouterWithFallback(prompt, systemPrompt = "", options = {}) {
    ...
    for (const modelName of DEFAULT_MODELS) {
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            ...
            if (!apiKey) {
                throw new Error('Yapay zeka (OpenRouter) API anahtarı ayarlanmamış. Lütfen .env.local dosyasına geçerli bir anahtar ekleyin.');
            }
